function disp_field = my_constrained_diffeomorphism(F, M, vx, vy, opts)
    ...
    y = 0:size(F,1)-1;
    z = 1 ./ sqrt(1 + (y./(2000*514/16000)).^12);
...
%         update_field_x = -1 * ((I_diff .* G_fix_x) .* cost_adjustment_mat) ./ ...
%             ((G_fix_mag.^2 + J.^2)/2 + I_diff.^2);
%         
%         update_field_y = -1 * ((I_diff .* G_mov_y)  .* cost_adjustment_mat) ./ ...
%             ((G_fix_mag.^2 + J.^2)/2 + I_diff.^2);
        
        update_field_x = -1 * ((((M_tilda - F).*cost_adjustment_mat).*G_fix_x) ./ ...
                ((opts.alpha).^2 * (M_tilda - F).^2 + G_fix_mag.^2) ... 
                + (((M_tilda - F).*cost_adjustment_mat).*G_mov_x) ./ ...
                ((opts.alpha).^2 * (M_tilda - F).^2 + G_mov_mag.^2));
        
        update_field_y = -1 * ((((M_tilda - F).*cost_adjustment_mat).*G_fix_y) ./ ...
                ((opts.alpha).^2 * (M_tilda - F).^2 + G_fix_mag.^2) ... 
                + (((M_tilda - F).*cost_adjustment_mat).*G_mov_y) ./ ...
                ((opts.alpha).^2 * (M_tilda - F).^2 + G_mov_mag.^2));
