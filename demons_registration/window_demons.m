function disp_field = window_demons(F, M, vx, vy, opts)
    ...
    if ~isfield(opts,'sigma_fluid');     opts.sigma_fluid   = 2.5;          end
    if ~isfield(opts,'sigma_diff');      opts.sigma_diff    = 2.5;          end
    ...
    stitch_dfx = imgaussfilt(stitch_dfx,2.0);
    stitch_dfy = imgaussfilt(stitch_dfy,2.0);
